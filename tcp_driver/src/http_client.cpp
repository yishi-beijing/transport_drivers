...
namespace drivers
{
  namespace tcp_driver
  {
    ...
    std::size_t HttpClient::write(std::string target, http::verb method, std::string body)
    {
      try
      {
...
#ifdef WITH_DEBUG_STDCOUT_HTTP_CLIENT
        {
          ...
          oss << "Request message " << req;
          std::cout << (oss.str());
          RCLCPP_INFO(rclcpp::get_logger("HttpClient::write"), "Request message: %s", oss.str().c_str());
          oss.clear();
        }
#endif
    ...
    void HttpClient::asyncWrite(Functor func, std::string target, http::verb method, std::string body)
    {
      try
      {
        ...
        RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncWrite"), "asyncWrite: %s : %s", target.c_str(), body.c_str());
        RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncWrite"), "m_remote_host: %s", m_remote_host.c_str());
#endif
        m_func = func;
        ...
        std::size_t bytes_transferred)
    {
      ...
      {
#ifdef WITH_DEBUG_STDCOUT_HTTP_CLIENT
        RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncOnRead(m_recv_string)"), "m_func: %s", m_recv_string.c_str());
#endif
        m_func(m_recv_string);
      }else{
#ifdef WITH_DEBUG_STDCOUT_HTTP_CLIENT
        RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncOnRead(m_res_string)"), "m_func: %s", m_res_string.c_str());
#endif
        m_func(m_res_string);
      }
    }
  ...
  // Once
  /*
    void HttpClient::asyncOnceWrite(Functor func, std::string target, http::verb method, std::string body)
    {
      try
      {
//        std::cout << m_remote_host << std::endl;
        ...
        std::shared_ptr<tcp::socket> socket_ = std::make_shared<tcp::socket>(*m_ctx);
    ...
    void HttpClient::asyncOnceGet(Functor func, std::string target)
    {
      RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncGet"), "asyncGet: %s", target.c_str());
      return HttpClient::asyncWrite(func, target, http::verb::get);
    }

    void HttpClient::asyncOncePost(Functor func, std::string target, std::string body)
    {
      RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncPost"), "asyncPost: %s", body.c_str());
      return HttpClient::asyncWrite(func, target, http::verb::post, body);
    }

    void HttpClient::asyncOnceOnResolve(
        beast::error_code ec,
        tcp::resolver::results_type results,
    ...
    void HttpClient::asyncOnceOnConnect(boost::system::error_code ec,
        std::shared_ptr<tcp::socket> socket)
    {
      RCLCPP_INFO(rclcpp::get_logger("HttpClient::asyncOnConnect"), "asyncOnConnect");
      if (ec)
...
//      http::async_write(m_socket, m_req,
    ...
    void HttpClient::asyncOnceOnWrite(
        boost::system::error_code ec,
        std::size_t bytes_transferred,
        std::shared_ptr<tcp::socket> socket)
    {
...
//      http::async_read(m_socket, m_recv_buffer, m_res,
    ...
    void HttpClient::asyncOnceOnRead(
        boost::system::error_code ec,
        std::size_t bytes_transferred,
        std::shared_ptr<tcp::socket> socket)
    {
