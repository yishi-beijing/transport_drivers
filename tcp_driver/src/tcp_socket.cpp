...
/// @return True if the socket state requires a reset.
        ...
        void TcpSocket::asyncReceiveHeaderPayload(Functor func_header, Functor func_payload,
        ...
        void TcpSocket::asyncReceiveHeaderPayloadRetry(std::vector<unsigned char> &buff, Functor func_header,
        ...
        void TcpSocket::asyncSendReceiveHeaderPayload(std::vector<unsigned char> &buff, Functor func_header,
                                                      Functor func_payload, std::function<void()> func_finally) {
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
          std::cout << "boost::asio::async_write" << std::endl;
#endif
              ...
              *m_socket,
              ...
              [this, func_header, func_payload, func_finally, buff](boost::system::error_code ec,
                  ...
                  if (needReset()) {
                    close();
                    if (!open()) {
                      RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpDriver::asyncSendReceiveHeaderPayload"),
                                          "socket open failed");
                      return;
                    }
                    reset_flg = false;
...
#endif
          if (error) {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpSocket::asyncReceiveHandlerHeaderPayloadRetry"),
                                error.message());
            reset_flg = true;
            if (needReset()) {
              close();
              if (!open()) {
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpDriver::asyncReceiveHandlerHeaderPayloadRetry"),
                                    "socket open failed");
                return;
              }
              reset_flg = false;
