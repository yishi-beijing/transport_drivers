...
{
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
  std::cout << "void TcpSocket::asyncSend(std::vector<unsigned char> & buff, std::function<void()> callback)" << std::endl;
#endif
  boost::asio::async_write(
          *m_socket,
          boost::asio::buffer(buff, buff.size()),
...
void TcpSocket::asyncReceive(Functor func)
{
  ...
  boost::asio::async_read(
      *m_socket,
      ...
      std::bind(&TcpSocket::asyncReceiveHandler, this,
                  std::placeholders::_1, std::placeholders::_2));
}

void TcpSocket::asyncReceiveHeaderPayload(Functor func_header, Functor func_payload, std::function<void ()>  func_finally)
{
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
  ...
  boost::asio::async_read(
      *m_socket,
      ...
      std::bind(&TcpSocket::asyncReceiveHandlerHeaderPayload, this,
                  std::placeholders::_1, std::placeholders::_2));
}

void TcpSocket::asyncReceiveHeaderPayloadRetry(std::vector<unsigned char> & buff, Functor func_header, Functor func_payload, std::function<void ()>  func_finally)
{
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
  ...
  boost::asio::async_read(
      *m_socket,
      m_recv_buffer,
      boost::asio::transfer_exactly(8),//for Hesai
      std::bind(&TcpSocket::asyncReceiveHandlerHeaderPayloadRetry, this,
                  std::placeholders::_1, std::placeholders::_2));
}
...
void TcpSocket::asyncSendReceiveHeaderPayload(std::vector<unsigned char> & buff, Functor func_header, Functor func_payload, std::function<void ()>  func_finally)
{
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
  std::cout << "boost::asio::async_write" << std::endl;
#endif
  boost::asio::async_write(
          *m_socket,
          boost::asio::buffer(buff, buff.size()),
          [this, func_header, func_payload, func_finally, buff](boost::system::error_code ec, std::size_t bytes_transferred)
          {
#ifdef WITH_DEBUG_STDCOUT_TCP_SOCKET
            std::cout << "async_write, func" << std::endl;
#endif
            ...
            if(needReset()){
              close();
              if(!open()){
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpDriver::asyncSendReceiveHeaderPayload"), "socket open failed");
                return;
              }
              reset_flg = false;
  ...
  std::size_t bytes_transferred)
{
  ...
  (void)bytes_transferred;
  if (error) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpSocket::asyncReceiveHandlerHeaderPayload"), error.message());
    reset_flg = true;
    return;
  }
          ...
          *m_socket,
          ...
          boost::asio::transfer_exactly(payload_num),
                ...
                std::size_t bytes_transferred)
            {
              (void)bytes_transferred;
              if (error) {
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpSocket::asyncReceiveHandlerHeaderPayload(async_read)"), error.message());
                reset_flg = true;
                return;
              }
...
#endif
  if (error) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpSocket::asyncReceiveHandlerHeaderPayloadRetry"), error.message());
    reset_flg = true;
    if(needReset()){
      close();
      if(!open()){
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("TcpDriver::asyncReceiveHandlerHeaderPayloadRetry"), "socket open failed");
        return;
      }
      reset_flg = false;
